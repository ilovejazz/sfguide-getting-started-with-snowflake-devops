...
rsaDir = pathlib.Path.home().joinpath('snowflake-rsa')

private_key_file = rsaDir.joinpath('rsa_key.p8')

def get_pass_phrase(rsaDir):
    
    f = open(rsaDir.joinpath('passPhrase.json'), "r")
    data = json.loads(f.read())
    f.close()
    
    return data['snowflakePassPhrase']


private_key_file_pwd = get_pass_phrase(rsaDir)
    
conn_params = {
    'account': 'MANDG-CTORND',
    'user': 'HUGHGREGOIRE',
